...
## Background on Ensemble Random Forests
...
knitr::kable(prop.table(table(simData$samples$obs)), col.names=c('Obs','Prop.'))
```
...
Now that we have covered the background, let's run an ERF. This is simple using `ens_random_forests`. 
```{r erf_ex}
...
## Next steps
Other tutorials will walk you through summarizing and visualing the results as well as discuss the various model performance metrics and their interpretation. 